...
async function translateText(obj, target) {
  ...
  console.log("translateText: received obj " + JSON.stringify(obj));
  let [translations] = await translate.translate(obj.text, target);
  translations = Array.isArray(translations) ? translations : [translations];
  console.log("Translations:");
  translations.forEach((translation, i) => {
    console.log(`${obj.text[i]} => (${target}) ${translation}`);
    console.log("Translated text: " + translations);
    io.sockets.in(obj.target.email).emit("event", {
      lang: target,
      text: translations,
      sourceSpeaker: obj.source,
    });
  });
}
...
const uri =
  "mongodb+srv://sujoy:Joyita16@cluster0.gnsesmw.mongodb.net/?retryWrites=true&w=majority";
...
client.connect((err) => {
  ...
  console.log("Successfully connected to MongoDB - ChatDB");
  //client.close();
});
...
app.post("/setLang", (req, res) => {
  ...
  if (!email || email.length < 4 || !lang || lang.length < 2) {
    console.log("setLang: inadequate params.");
    return;
  }
  console.log("Received setLang request: " + JSON.stringify(req.body));
  for (i = 0; i < loggedinUsers.length; i++) {
    if (loggedinUsers[i].email == email) {
      ...
      console.log(
        "New loggedinUser JSON = " + JSON,
        stringify(loggedinUsers[i])
      );
      break;
    }
  }
...
io.on("connection", function (socket) {
  ...
  socket.on("speech", function (speech) {
    ...
    for (i = 0; i < loggedinUsers.length; i++) {
      if (loggedinUsers[i].email == speech.target.email) {
        console.log("Found email match for target in speech event");
        targetLang = loggedinUsers[i].preferred_lang;
        break;
      }
    }
    try {
      if (targetLang && targetLang.length > 1)
        //translateText(speech.text, targetLang, speech.target.email);
        translateText(speech, targetLang);
      else {
        console.log("No preferred targetLang found, resorting to default Thai");
        //translateText(speech.text, "th", speech.target.email);
        translateText(speech, "th");
      }
    } catch (err) {
